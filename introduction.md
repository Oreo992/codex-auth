...
目前市面上已有一些比较火热的工具来解决这个问题。

比如 **CPA（ChatGPT Proxy Admin）** 之类的方案，它的思路是这样的：

1. 你需要自己单独部署一个代理服务
2. 在这个服务上面登录你的 ChatGPT 账号
3. 由服务自动做负载均衡和轮询，你不需要手动操心账号切换

看起来挺省心的，对吧？

但仔细一想，**问题其实不少**：

**第一，安全风险。** 你的账号凭证需要交给一个中间服务来管理。不管它是自建的还是别人搭建的，你的 Token 和登录信息都暴露在了一个额外的环节上。万一这个服务被攻破、日志泄露，你的账号就危险了。对于正规购买的账号来说，这是一个不小的安全隐患。

**第二，需要额外的服务器。** 要跑这类代理服务，你得有一台能访问 OpenAI 的服务器。这就排除了不少人——比如我自己，手头只有香港的服务器。而 OpenAI 本身不对香港提供服务，所以即使我想搭建，也没法用。

**第三，配置繁琐。** 部署服务、配置反向代理、管理进程守护、处理 SSL 证书……对于只想安安静静写代码的人来说，这些额外的运维工作实在是太折腾了。

**那有没有一种更简单、更安全的方式？**

**不需要服务器、不需要搭建任何服务、不需要做复杂配置，纯粹本地化的命令行工具？**
...
**Codex Auth** 是一个纯本地的命令行工具（CLI），专门用来安全切换和管理多个 Codex 账号。项目完全开源，MIT 协议。
...
### 2. `codex-auth add` — 添加当前登录的账号

```shell
codex-auth add            # 默认会先执行 codex login
codex-auth add --no-login # 跳过登录，直接读取本地认证文件
```

`add` 命令会把当前 `~/.codex/auth.json` 中的账号导入到 Codex Auth 的管理列表中。

默认行为是先调起 `codex login`（也就是 Codex 自己的登录流程），登录完成后再自动读取认证文件并添加。

**但你也可以完全不走这个流程。** 加上 `--no-login` 参数，Codex Auth 会直接读取已有的 `auth.json` 文件。如果你已经在 Codex 里登好了账号，用这种方式最方便——不需要再登录一次。
...
导入一份认证文件，还可以用 `--name` 给它起个别名。别名会在 `list` 和 `switch` 命令中显示在邮箱前面，方便区分。比如设了 `personal`，列表里就会显示 `(personal)user@example.com`。
...
无效的文件会被自动跳过，不会报错。批量导入模式下 `--name` 参数会被忽略，因为每个账号会根据邮箱自动命名。
...
codex-auth import <文件路径> --name <别名> # 导入并设置别名
